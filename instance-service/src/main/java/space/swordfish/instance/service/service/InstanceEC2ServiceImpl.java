...
public class InstanceEC2ServiceImpl implements InstanceEC2Service {
	...
	@Autowired
	public InstanceEC2ServiceImpl(InstanceRepository instanceRepository,
					...
					public void onSuccess(StartInstancesRequest request,
						...
						for (InstanceStateChange stateChange : instanceStateChanges) {
							...
							amazonEC2Async.waiters().instanceRunning().runAsync(
									describeInstancesRequestWaiterParameters(
											stateChange.getInstanceId()),
									describeInstancesRequestWaiterHandler());
						}
					}
				});
	}
	...
	/**
	 * Fetch all data from AWS and either syncByInstanceId it as a new instance locally,
	 * or update a local instance with new data.
	 */
	@Override
	public void syncAll() {
	...
	private WaiterParameters<DescribeInstancesRequest> describeInstancesRequestWaiterParameters(
			String instanceId) {

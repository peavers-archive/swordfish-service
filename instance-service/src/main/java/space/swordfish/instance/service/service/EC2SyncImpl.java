...
public class EC2SyncImpl extends EC2BaseService implements EC2Sync {
  ...
  public List<String> instancesNotOnAmazon() {
    ...
    List<com.amazonaws.services.ec2.model.Instance> remoteInstances =
        response.getReservations().get(0).getInstances();
    ...
    for (Reservation reservation : response.getReservations()) {
      for (com.amazonaws.services.ec2.model.Instance instance : reservation.getInstances()) {
        ...
        if (instance.getState().getName().equals("terminated")
            || !instance.getTags().contains(new Tag().withKey("Swordfish").withValue("true"))) {
          continue;
        }
  ...
  private Instance getInstanceDetails(com.amazonaws.services.ec2.model.Instance awsInstance) {
    // If we've got extra data such as keys or names in the database grab em, otherwise create a new
    // instance

...
public class SilverstripeDeploymentImpl implements SilverstripeDeployment {
  ...
  public ResponseEntity<JsonNode> create(
      String projectId, String environmentId, DeploymentEvent deploymentEvent) {
    return authenticatedRestTemplate
        .restTemplate()
        .exchange(
            HOST + "/{projectId}/environment/{environmentId}/deploys",
            HttpMethod.POST,
            new HttpEntity<>(deploymentEvent),
            JsonNode.class,
            projectId,
            environmentId);
  }
  ...
  @Override
  public ResponseEntity<JsonNode> view(
      String projectId, String environmentId, String deploymentId) {
    return authenticatedRestTemplate
        .restTemplate()
        .exchange(

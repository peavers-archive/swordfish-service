...
public class SilverstripeServiceImpl implements SilverstripeService {
    ...
    @Autowired
    public SilverstripeServiceImpl(SilverstripeStack silverstripeStack, SilverstripeSnapshot silverstripeSnapshot, JsonTransformService jsonTransformService, NotificationService notificationService) {
    ...
    public List<Stack> getAllStacks() {
        return jsonTransformService.readList(Stack.class, silverstripeStack.listAll().getBody().toString()).get();
    }
    ...
    public Future<String> createSnapshot(String projectId, StackEvent stackEvent) {
        ...
        Callable<String> task = () -> {
            Transfer transfer = jsonTransformService.read(Transfer.class, silverstripeSnapshot.create(stackEvent.getProjectId(), stackEvent).getBody().toString());
            Transfer transferProgress = transferProgress(silverstripeSnapshot.transfer(projectId, transfer.getId()));

            while (!transferProgress.getStatus().equals("Finished")) {

                transferProgress = transferProgress(silverstripeSnapshot.transfer(projectId, transfer.getId()));
                log.info("transfer state {}", transferProgress);
                notificationService.send("restore_event", "restore_info", "Snapshot building for " + projectId);
            ...
            notificationService.send("restore_event", "restore_success", "Snapshot created for " + projectId);
